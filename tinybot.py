...
#### 1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l
    ...
    def on_join(self, join_info):
        ...
        #if self.active_user.account not in knownlist:
            #self.do_ban(self.active_user.nick)
            #self.send_chat_msg("Guest - %s not allowed." % self.active_user.account)
        if not _user.account in knownlist:
            #self.send_chat_msg('%s not known.' % _user.account )
            print('========== %s not known.' % _user.account)

            if pinylib.CONFIG.B_CheckUser:
                self.send_chat_msg('%s not VIP - use !knownadd %s' %  (_user.nick, _user.account))

            if pinylib.CONFIG.B_vip:
                #self.do_bad_nick(self.active_user.nick)
                self.do_kick(_user.nick)
                self.send_chat_msg('VIP to %s Denied - use !knownadd %s' %  (_user.nick, _user.account))
                print('========== VIP Denied - %s' % _user.account)

        # If account is in known.txt print, sending to chat will get kicked for spam.
        if _user.account in knownlist:
            #self.send_chat_msg('%s is known.' % _user.account )
            print('========== %s is known.' % _user.account)

        # If a nickname starts with guest-  such as guest-0001
        ...
        if _user.account:
            ...
            if _user.is_owner or _user.account in adminlist:
                _user.user_level = 1
                self.console_write(pinylib.COLOR['red'], 'Room Owner %s:%d:%s' % (_user.nick, _user.id, _user.account))
                if not _user.account in knownlist:
                ...
                _user.user_level = 3
                self.console_write(pinylib.COLOR['bright_red'], 'Moderator %s:%d:%s' % (_user.nick, _user.id, _user.account))
                if not _user.account in knownlist:

                    pinylib.file_handler.file_writer(self.config_path, pinylib.CONFIG.B_known, _user.account)
                    self.send_chat_msg('Mod saved to whitelist: %s' % _user.account)
                

            else:
                self.console_write(pinylib.COLOR['bright_yellow'], '%s:%d has account: %s' % (_user.nick, _user.id, _user.account))
            ...
            if pinylib.CONFIG.B_GREET and self.is_client_mod:
                ...
                if not _user.nick.startswith('guest-'):
                    ...
                    if _user.account:
                    	...
                    	self.send_chat_msg('Welcome ' + _user.nick)
                    else:
                    	#self.do_ban(_user.nick)
                        #self.send_chat_msg(heart + 'Welcome to the room %s:%s' % (_user.nick, _user.id))
                       	self.send_chat_msg('Welcome ' + _user.nick)
    ...
    def message_handler(self, msg):
        ...
        if len(msg) > 250:
            self.do_ban(self.active_user.nick)
            #self.do_bad_nick(self.active_user.nick)
            self.send_chat_msg("Spam Detected (%s)!" % len(msg))
        

        #if msg.startswith(unicode('⭕', 'utf-8')):
        	#self.do_ban(self.active_user.nick)
	    ...
	    #admin/botowner
            ...
            if (pinylib.CONFIG.B_PUBLIC_CMD and self.has_level(5)) or self.active_user.user_level < 5:
                ...
                elif cmd == prefix + 'rules':

                	self.send_chat_msg("https://pastebin.com/UPFcaqck")
