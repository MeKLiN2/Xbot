...
class CheckUser:
    ...
    def check_account(self):
        ...
        if self.user.account in self.config.B_ACCOUNT_BANS:
            ...
            if self.config.B_USE_KICK_AS_AUTOBAN:
                self.tinybot.send_chat_msg('Auto-Kicked: (cucked)')
            else:
                self.tinybot.send_chat_msg('Auto-Banned: (cucked)')
    ...
    def guest_entry(self):
        ...
        if self.user.account == '' and not self.config.B_ALLOW_GUESTS:
            ...
            if self.config.B_USE_KICK_AS_AUTOBAN:
                self.tinybot.send_chat_msg('Auto-Kicked: (no guests)')
            else:
                self.tinybot.send_chat_msg('Auto-Banned: (no guests)')
    ...
    def check_nick(self):
        ...
        if self.user.nick in self.config.B_NICK_BANS:
            ...
            if self.config.B_USE_KICK_AS_AUTOBAN:
                self.tinybot.send_chat_msg('Auto-Kicked: (some cuck)')
            else:
                self.tinybot.send_chat_msg('Auto-Banned: (some cuck)')
    ...
    def check_lurker(self):
        ...
        if self.user.is_lurker and not self.config.B_ALLOW_LURKERS:
            ...
            if self.config.B_USE_KICK_AS_AUTOBAN:
                self.tinybot.send_chat_msg('Auto-Kicked: (captcha)')
            else:
                self.tinybot.send_chat_msg('Auto-Banned: (captcha)')
